...
import { Button } from "@/components/ui/button" // Updated to use @
import { Card } from "@/components/ui/card"     // Updated to use @
...
export function ProductCard({ id, name, price, weight, image, category }: ProductCardProps) {
  // If useCart or useFavorites are missing, we add a fallback to prevent crash
  const cart = useCart ? useCart() : { addItem: () => {} }
  const favorites = useFavorites ? useFavorites() : { isFavorite: () => false, toggleFavorite: () => false }
  
  const addItem = cart.addItem
  const { isFavorite, toggleFavorite } = favorites
  const isProductFavorite = isFavorite ? isFavorite(id) : false
